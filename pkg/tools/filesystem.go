...
func (t *ReadFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	offset := 0
	if v, ok := args["offset_bytes"].(float64); ok {
		offset = int(v)
	}
	if offset < 0 {
		offset = 0
	...
	hashPrefix := fmt.Sprintf("%x", sum[:6])

	out := string(chunk)
	if truncated {
		out += fmt.Sprintf("\n\n...(truncated; offset_bytes=%d; max_bytes=%d; total_bytes=%d; sha256_prefix=%s)",
			offset, maxBytes, len(content), hashPrefix)
	}

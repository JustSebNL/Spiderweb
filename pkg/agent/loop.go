...
import (
	...
	"github.com/JustSebNL/Spiderweb/pkg/utils"
)
...
type AgentLoop struct {
	...
	cheapCognition        *cognition.Client
	cheapCalls            atomic.Int64
	cheapFailures         atomic.Int64
	cheapForwarded        atomic.Int64
	cheapSkipped          atomic.Int64
...
func NewAgentLoop(cfg *config.Config, msgBus *bus.MessageBus, provider providers.LLMProvider) *AgentLoop {
	...
	var valve *inboundValve
	if cfg != nil && cfg.Intake.Enabled {
		valve = newInboundValve(cfg.Intake)
	}

	var fwdURL string
	var fwdClient *http.Client
	if cfg != nil && cfg.Intake.ForwardURL != "" {
		fwdURL = cfg.Intake.ForwardURL
		timeout := time.Duration(cfg.Intake.ForwardTimeout) * time.Second
		if timeout <= 0 {
			timeout = 5 * time.Second
		}
		fwdClient = &http.Client{Timeout: timeout}
		logger.InfoCF("agent", "Intake forwarder enabled", map[string]any{
			"url": fwdURL,
		})
	}

	var cheapClient *cognition.Client
	if cfg != nil && cfg.Intake.CheapCognition.Enabled {
		if client, err := cognition.NewClientFromConfig(cfg); err != nil {
			logger.WarnCF("agent", "Cheap cognition setup failed", map[string]any{"error": err.Error()})
		} else {
			cheapClient = client
...
func (al *AgentLoop) Run(ctx context.Context) error {
	...
	next := func(ctx context.Context) (bus.InboundMessage, bool) {
		return al.bus.ConsumeInbound(ctx)
	}

	if al.inboundValve != nil {
		inbox := al.inboundValve.Start(ctxRun, al.bus)
		next = func(ctx context.Context) (bus.InboundMessage, bool) {
			select {
			case msg, ok := <-inbox:
				return msg, ok
...
func (al *AgentLoop) forwardMessage(ctx context.Context, msg bus.InboundMessage) {
	...
	if err != nil {
		logger.ErrorCF("agent", "Forward marshal failed", map[string]any{"error": err.Error()})
		return
	}

	req, err := http.NewRequestWithContext(ctx, "POST", al.forwardURL, bytes.NewReader(body))
	if err != nil {
		logger.ErrorCF("agent", "Forward request build failed", map[string]any{"error": err.Error()})
		return
	}
	req.Header.Set("Content-Type", "application/json")

	resp, err := al.forwardClient.Do(req)
	if err != nil {
		logger.WarnCF("agent", "Forward to OpenClaw failed", map[string]any{
			"error": err.Error(),
			"url":   al.forwardURL,
		})
		return
	...
	if resp.StatusCode >= 300 {
		logger.WarnCF("agent", "OpenClaw returned non-OK", map[string]any{
	...
	logger.DebugCF("agent", "Message forwarded to OpenClaw", map[string]any{
		"channel": msg.Channel,
		"chat_id": msg.ChatID,
		"status":  resp.StatusCode,
	})
...
func (al *AgentLoop) processMessage(ctx context.Context, msg bus.InboundMessage) (string, error) {
	...
	enableSummary := false
	if al.cfg != nil {
		enableSummary = al.cfg.Session.AutoSummarize
	}
...
func (al *AgentLoop) classifyForwardMessage(
	...
	result, err := al.cheapCognition.ClassifyEvent(classifyCtx, cognition.Event{
		Source:       msg.Channel,
		SourceObject: msg.ChatID,
		...
		Metadata: stringMapToAnyMap(msg.Metadata),
	})
	if err != nil {
		al.cheapFailures.Add(1)
	...
	al.cheapLastLatencyMS.Store(time.Since(startedAt).Milliseconds())
	al.cheapLastDecisionUnix.Store(time.Now().Unix())
	al.cheapLastError.Store("")
	decision := "forward"
	priority := strings.ToLower(strings.TrimSpace(result.Priority))
	if !result.EscalationNeeded && (priority == "low" || priority == "routine" || priority == "info") {
		decision = "skip"
	}
...
func (al *AgentLoop) CheapCognitionSnapshot() maintenance.CheapCognitionSnapshot {
	snapshot := maintenance.CheapCognitionSnapshot{
		ClassificationCalls: al.cheapCalls.Load(),
		ClassificationFails: al.cheapFailures.Load(),
		Forwarded:           al.cheapForwarded.Load(),
		Skipped:             al.cheapSkipped.Load(),
		LastLatencyMS:       al.cheapLastLatencyMS.Load(),
	}
	if lastError, ok := al.cheapLastError.Load().(string); ok {
		snapshot.LastError = lastError
	}
	if ts := al.cheapLastDecisionUnix.Load(); ts > 0 {
		snapshot.LastDecisionAt = time.Unix(ts, 0).UTC()
	}
	return snapshot
}

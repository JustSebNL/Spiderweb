...
Spiderweb's tools configuration is located in the `tools` field of `config.json`.
...
By default, Spiderweb blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `SPIDERWEB_TOOLS_<SECTION>_<KEY>`:

For example:
- `SPIDERWEB_TOOLS_WEB_BRAVE_ENABLED=true`
- `SPIDERWEB_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `SPIDERWEB_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`

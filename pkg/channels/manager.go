...
func (m *Manager) initChannels() error {
	...
	if m.config.Channels.OpenClaw.Enabled {
		logger.DebugC("channels", "Attempting to initialize OpenClaw bridge channel")
		openclaw, err := NewOpenClawChannel(m.config.Channels.OpenClaw, m.bus)
		if err != nil {
			logger.ErrorCF("channels", "Failed to initialize OpenClaw bridge channel", map[string]any{
				"error": err.Error(),
			})
		} else {
			m.channels["openclaw"] = openclaw
			logger.InfoC("channels", "OpenClaw bridge channel enabled successfully")
		}
	}

...
func executeConfigMigration(srcConfigPath, dstConfigPath, picoClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing Spiderweb config: %w", err)
		}
...
func resolveSpiderwebHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("SPIDERWEB_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".spiderweb"), nil
}

# Using Antigravity Provider in Spiderweb

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Spiderweb.
...
```bash
spiderweb auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where Spiderweb is waiting.

Spiderweb will extract the authorization code and complete the process automatically.
...
```bash
spiderweb auth models
```

### Switch Models
You can change the default model in `~/.spiderweb/config.json` or override it via the CLI:
...
2.  **Authentication persistence**: 
    ...
    ```bash
    scp ~/.spiderweb/auth.json user@your-server:~/.spiderweb/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. Spiderweb will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `spiderweb auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.

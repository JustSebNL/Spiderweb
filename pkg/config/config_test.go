...
func TestDefaultConfig_CheapCognitionDefaults(t *testing.T) {
	cfg := DefaultConfig()
	if cfg.Intake.CheapCognition.Enabled {
		t.Fatal("DefaultConfig().Intake.CheapCognition.Enabled should be false")
	}
	if cfg.Intake.CheapCognition.Runtime != "auto" {
		t.Fatal("DefaultConfig().Intake.CheapCognition.Runtime should be auto")
	}
	if cfg.Intake.CheapCognition.BaseURL == "" {
		t.Fatal("DefaultConfig().Intake.CheapCognition.BaseURL should not be empty")
	}
	if cfg.Intake.CheapCognition.Model == "" {
		t.Fatal("DefaultConfig().Intake.CheapCognition.Model should not be empty")
	}
	if cfg.Intake.CheapCognition.TimeoutSeconds <= 0 {
		t.Fatal("DefaultConfig().Intake.CheapCognition.TimeoutSeconds should be > 0")
	}
}
...
func TestLoadConfig_RuntimeEnvFromConfigDir(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.json")
	runtimeEnvPath := filepath.Join(tmpDir, "runtime.env")

	keys := []string{
		"SPIDERWEB_RUNTIME_ENV",
		"SPIDERWEB_INTAKE_CHEAP_COGNITION_ENABLED",
		"SPIDERWEB_INTAKE_CHEAP_COGNITION_RUNTIME",
		"SPIDERWEB_INTAKE_CHEAP_COGNITION_BASE_URL",
		"SPIDERWEB_INTAKE_CHEAP_COGNITION_MODEL",
	}
	...
	if err := os.WriteFile(configPath, []byte(`{"intake":{"cheap_cognition":{"enabled":false}}}`), 0o600); err != nil {
		t.Fatalf("WriteFile(config) error: %v", err)
	}
	if err := os.WriteFile(runtimeEnvPath, []byte("# generated by bootstrap\nSPIDERWEB_INTAKE_CHEAP_COGNITION_ENABLED=1\nSPIDERWEB_INTAKE_CHEAP_COGNITION_RUNTIME=llama_cpp\nSPIDERWEB_INTAKE_CHEAP_COGNITION_BASE_URL=http://127.0.0.1:8081/v1\nSPIDERWEB_INTAKE_CHEAP_COGNITION_MODEL=tencent/Youtu-LLM-2B-GGUF:Youtu-LLM-2B-Q8_0.gguf\n"), 0o600); err != nil {
		t.Fatalf("WriteFile(runtime env) error: %v", err)
	}
	...
	if !cfg.Intake.CheapCognition.Enabled {
		t.Fatal("Cheap cognition should be enabled by runtime.env")
	}
	if cfg.Intake.CheapCognition.Runtime != "llama_cpp" {
		t.Fatalf("Runtime = %q, want llama_cpp", cfg.Intake.CheapCognition.Runtime)
	}
	if cfg.Intake.CheapCognition.BaseURL != "http://127.0.0.1:8081/v1" {
		t.Fatalf("BaseURL = %q", cfg.Intake.CheapCognition.BaseURL)
	}
	if cfg.Intake.CheapCognition.Model != "tencent/Youtu-LLM-2B-GGUF:Youtu-LLM-2B-Q8_0.gguf" {
		t.Fatalf("Model = %q", cfg.Intake.CheapCognition.Model)
	}
}

func TestLoadConfig_RuntimeEnvOverridePath(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.json")
	overrideEnvPath := filepath.Join(tmpDir, "override.env")
	...
	if err := os.Setenv("SPIDERWEB_RUNTIME_ENV", overrideEnvPath); err != nil {
		t.Fatalf("Setenv(runtime env path) error: %v", err)
	}

	if err := os.WriteFile(configPath, []byte(`{}`), 0o600); err != nil {
		t.Fatalf("WriteFile(config) error: %v", err)
	}
	if err := os.WriteFile(overrideEnvPath, []byte("export SPIDERWEB_INTAKE_CHEAP_COGNITION_BASE_URL=http://127.0.0.1:9000/v1\n"), 0o600); err != nil {
		t.Fatalf("WriteFile(runtime env) error: %v", err)
	}
	...
	if cfg.Intake.CheapCognition.BaseURL != "http://127.0.0.1:9000/v1" {
		t.Fatalf("BaseURL = %q, want override value", cfg.Intake.CheapCognition.BaseURL)
	}
}

func TestLoadConfig_RuntimeEnvWithoutConfigFile(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "config.json")
	runtimeEnvPath := filepath.Join(tmpDir, "runtime.env")
	...
	if err := os.WriteFile(runtimeEnvPath, []byte("SPIDERWEB_INTAKE_CHEAP_COGNITION_ENABLED=1\n"), 0o600); err != nil {
		t.Fatalf("WriteFile(runtime env) error: %v", err)
	}
	...
	if !cfg.Intake.CheapCognition.Enabled {
		t.Fatal("Cheap cognition should load from runtime.env even without config.json")
	}
}
